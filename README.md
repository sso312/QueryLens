...
| **Prompt** | 프롬프트 수정 | txt2SQL 정확도를 높이기 위한 시스템 프롬프트나 지시문 수정 |
| **RAG** | 검색 엔진 작업 | 벡터 DB 구축, 문서 임베딩 과정(Embedding) 또는 검색 로직 수정 |
| **Data** | 데이터 전처리 | SQL 학습 데이터셋 추가, DB 스키마(Table) 정보 업데이트 |
| **SQL** | 쿼리 로직 수정 | 생성된 SQL의 문법 오류 수정 또는 쿼리 실행기(Executor) 코드 변경 |
| **Feat** | 기능 추가 | 챗봇 UI 생성, 새로운 API 엔드포인트 개발 등 기능 구현 |
| **Fix** | 버그 수정 | 코드 오류 해결, API 연결 끊김 현상 등 기술적 결함 수정 |
| **Docs** | 문서 수정 | README.md 작성, 프로젝트 기획서 또는 API 명세서 업데이트 |
| **Test** | 테스트 관련 | txt2SQL 정확도 평가(Evaluation) 코드 작성 및 테스트 실행 |

2. UI/UX 개발
| **말머리** | **의미** | 사용 경우 (Case) |
| --- | --- | --- |
| **Feat** | 기능 추가 | 새로운 기능을 처음 만들어서 넣었을 때 |
| **Fix** | 버그 수정 | 틀린 코드나 오류를 고쳤을 때 |
| **Design** | 디자인 변경 | 화면 레이아웃, 색상, 폰트 등을 수정했을 때 |
| **Docs** | 문서 수정 | README.md나 기획서 같은 문서를 작성/수정했을 때 |
| **Style** | 스타일 수정 | 기능 변화 없이 코드의 가독성이나 줄바꿈만 고쳤을 때 |
| **Rename** | 이름 변경 | 파일명이나 폴더명을 바꿨을 때 |
